...
function generateStaticArrows(player)
    ...
    for i = 0, keyCount-1 do 
        -- This forces the cycle: 0=Left, 1=Down, 2=Up, 3=Right
        local dirIndex = (i % 4) 
        local dirs = {'LEFT', 'DOWN', 'UP', 'RIGHT'}
        local arrowDir = dirs[dirIndex + 1]
        ...
        runHaxeCode([[
            ...
            if (player == 1) game.playerStrums.add(babyArrow);
            else {
                if(ClientPrefs.middleScroll) {
                    babyArrow.x += 310;
                    if(]]..i..[[ > ]]..((keyCount/2)-1)..[[) babyArrow.x += FlxG.width / 2 + 25;
                }
            ...
            // Use standard Psych Engine animation names
            babyArrow.animation.addByPrefix('static', 'arrow]]..arrowDir..[[');
            babyArrow.animation.addByPrefix('pressed', ']]..string.lower(arrowDir)..[[ press', 24, false);
            babyArrow.animation.addByPrefix('confirm', ']]..string.lower(arrowDir)..[[ confirm', 24, false);
            ...
            babyArrow.x += 50 + ]]..offset..[[ + ((FlxG.width / 2) * player);
            babyArrow.ID = ]]..i..[[;
