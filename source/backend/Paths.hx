...
class Paths
{
	...
	public static function cacheBitmap(key:String, ?parentFolder:String = null, ?bitmap:BitmapData, ?allowGPU:Bool = true):FlxGraphic
	{
		if (bitmap == null)
		{
			...
			if(!FileSystem.exists(file))
			{
				var assetName = extractName(file);

				// character fallback
				if(file.indexOf("characters") != -1)
				{
					RemoteAssets.ensureMaterial("characters", assetName);
				}

				// stage fallback
				if(file.indexOf("stages") != -1)
				{
					RemoteAssets.ensureMaterial("stages", assetName);
				}
			}
			if (bitmap == null)
			{
	...
	inline public static function getSparrowAtlas(key:String, ?parentFolder:String = null):FlxAtlasFrames
{
    var image:FlxGraphic = image(key, parentFolder);
    
    // THE PSYCH ENGINE WAY
    var xml:String = Paths.getPath('images/$key.xml', TEXT, parentFolder);
    
    // Check if the file exists in the mod folder or the base assets folder
    if (FileSystem.exists(xml) || Assets.exists(xml))
    {
        return FlxAtlasFrames.fromSparrow(image, Assets.getText(xml));
    }
    
    trace("CRITICAL: XML not found for " + key + " at path: " + xml);
    return null;
}
