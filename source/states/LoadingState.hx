...
static function checkAudioSources(song:backend.Song.SwagSong, onDone:Void->Void) {
	...
	static function ensureSongMaterials(song:SwagSong)
	{
		if(song == null) return;

		trace("Checking remote materials...");
		backend.OnlineSession.reset(); // Clear old ones

		// Characters
		if(song.player1 != null)
			RemoteAssets.ensureMaterial("characters", song.player1);

		if(song.player2 != null)
			RemoteAssets.ensureMaterial("characters", song.player2);

		if(song.gfVersion != null)
			RemoteAssets.ensureMaterial("characters", song.gfVersion);

		// Stage
		if(song.stage != null)
			RemoteAssets.ensureMaterial("stages", song.stage);
		if(Reflect.hasField(song, "req")) {
			var requirements:Array<String> = Reflect.field(song, "req");
			for(mat in requirements) {
				backend.OnlineSession.addMaterial(mat);
				backend.RemoteAssets.ensureMaterial("characters", mat);
			}
		}
	}
