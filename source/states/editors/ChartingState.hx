...
class ChartingState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	var curZoom:Float = 1;
	var reqInputText:FlxUIInputText; // Make sure this is here!
	...
	function onChartLoaded()
	{
		...
		var reqInputText:FlxUIInputText; // Changed from UIInputText to FlxUIInputText
	...
	function addChartingTab()
	{
		...
		objY  += 40;
        // --- NEW: REQUIRED MATERIALS INPUT ---
        // We place it 40 pixels below the Mute Inst checkbox
        var reqLabel = new FlxText(10, objY, 64, "Required Mats:");
        reqLabel.setFormat(null, 8, FlxColor.WHITE, LEFT);

        reqInputText = new FlxUIInputText(10, reqLabel.y + 15, 100, "");
        
        if(PlayState.SONG.req != null && PlayState.SONG.req.length > 0) {
            reqInputText.text = PlayState.SONG.req.join(", ");
        }

        // IMPORTANT: Add to the same group as the checkbox
        // If 'tab_group_song' is undefined, look at what 'check_mute_inst' is added to.
        // It might be 'tab_group' or 'temp_group'.
        tab_group.add(reqLabel);
        tab_group.add(reqInputText);
        // -------------------------------------
		objY += 50;
	...
	function addSongTab()
	{
		...
		var reloadJsonButton:PsychUIButton = new PsychUIButton(objX + 205, objY, 'Reload JSON', function()
		{
			...
			var reqLabel = new FlxText(10, 150, 64, "Required Mats:");
			reqLabel.setFormat(null, 8, FlxColor.WHITE, LEFT);
			...
			// IMPORTANT: Use 'tab_group_song' (or whatever the local variable is named)
			tab_group_song.add(reqLabel);
			tab_group_song.add(reqInputText);
	...
	function updateChartData()
	{
		...
		// --------------------------------------------------------
		// [NEW CODE] SAVE REQUIRED MATERIALS
		// --------------------------------------------------------
		if (reqInputText != null)
		{
			PlayState.SONG.req = []; // Reset array
			var rawReqs:Array<String> = reqInputText.text.split(",");
			
			for (r in rawReqs)
			{
				var clean:String = StringTools.trim(r);
				if (clean.length > 0)
				{
					PlayState.SONG.req.push(clean);
				}
			}
		}
		// --------------------------------------------------------
