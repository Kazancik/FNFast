...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		#if (LUA_ALLOWED || HSCRIPT_ALLOWED)
		// --- ONLINE MATERIAL SPECIFIC SCRIPTS ---
		// This only loads scripts if the material is actually in the song's requirements
		for (matName in backend.OnlineSession.activeMaterials)
		{
			// UPDATED: Use the direct assets path where we saved them
			var folderPath:String = 'assets/shared/scripts/' + matName + '/';
			
			if (sys.FileSystem.exists(folderPath))
			{
				for (file in sys.FileSystem.readDirectory(folderPath))
				{
					var fullPath:String = folderPath + file;

					#if LUA_ALLOWED
					if(file.toLowerCase().endsWith('.lua'))
					{
						new psychlua.FunkinLua(fullPath);
						trace("Loaded Material Lua: " + fullPath);
					}
					#end
					...
					if(file.toLowerCase().endsWith('.hx'))
					{
						...
						trace("Loaded Material HScript: " + fullPath);
					}
					#end
				}
			}
...
*/
    if (!cpuControlled && ClientPrefs.getGameplaySetting('songspeed') != 1 &&backend.ClientPrefs.authToken != ""  && healthGain == 1 && healthLoss == 1 && !practiceMode && ClientPrefs.getGameplaySetting('scrollspeed') == 1 && ClientPrefs.getGameplaySetting('scrolltype') == 'multiplicative' )
    {
        ...
        http.setPostData(haxe.Json.stringify(scoreData));
        http.setHeader("Content-Type", "application/json");
        
        http.onData = function(res) {
            trace("Score submitted! Response: " + res);
        };
        
        http.request(true); // Sends POST
    }
	else{
		trace("Your settings prevent score submission. To enable score submission, make sure health gain and loss are both set to 1, practice mode is off, scroll speed multiplier is 1, and scroll type is multiplicative. Also, make sure you are logged in with a valid account and the song has a valid chart ID.");
	}
